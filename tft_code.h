...
#define screen_refresh_time 250                      //промежуток между обновлениями экрана, в миллисекундах
...
public:
  ...
  void Draw_ignor_sign(byte current_page, bool draw);  //отображение значка игнорирования значений текущего параметра (только для страниц 3-8)


private:
  byte _page = 1;         //текущая страница
  bool _nextpage = true;  //флаг была ли изменена страница
  uint32_t _time_stamp;   //время подследнего обновления
  TFT* _p_TFT;            //указатель на класс TFT, чтобы использовать его команды

  bool _is_ignor;  //отображать изменение параметра текущей страницы или нет
  //Интересный момент: только у класса SCREEN будет поле, отвечающее за игнор значений какого-либо параметра,
  //так как только класс, отвечающий за отображение, знает отображать значение или нет. (так ли это?)
  //Или за это отвечает основная программа?
  //Просто нелогично в класс сенсоров добавлять поле игнорирования отображения, так как к датчикам это не относится.
  //
  //А ещё возник вопрос будем ли мы отображать игнор на первых двух страницах (по 3 параметра) и
  //будут ли там изменяться значения игнорируемых параметров.
  ...
  void Draw_value_borders();  //визуальное отображение положения текущего значения относительно установленных допустимых границ

  void Draw_danger_sign();  //отображение значка того, что значение параметра вышло за установленные допустимые границы
};
...
void SCREEN::PageDraw_1() {
  ...
  if (ignored_sensors_array[3] == false) {
    v_SHUM = String((*_p_DB).get_sensors_value(), 1);
  }
  ...
  //здесь должна быть процедура отрисовки значка тревоги, если параметр вышел за допустимые границы значений.
  //здесь должна быть процедура отрисовки значка уровня отклонения параметра от оптимального.
...
void SCREEN::Static_PageDraw_2() {
  ...
  (*_p_TFT).stroke(255, 0, 255);         //шрифт текста дальше =фиолетовый
  (*_p_TFT).setTextSize(1);              //размер текста дальше =1
  (*_p_TFT).text("Temperatura:", 0, 0);  //вывод текста "Osveschennost`:" с отступом от левого края = 0 и от верхнего = 0
  (*_p_TFT).setTextSize(0);              //вывод текста "°C" с отступом от левого края = 78 и от верхнего = 20:
  (*_p_TFT).stroke(255, 255, 255);
  (*_p_TFT).text("o", 78, 17);  //сначала "°", так как такой символ не поддерживается, а лучше - универсально
  (*_p_TFT).setTextSize(2);
  (*_p_TFT).text("C", 84, 19);
...
void SCREEN::PageDraw_2() {
  ...
  if (ignored_sensors_array[6] == false) {
    v_GAS = String((*_p_GAS).get_sensors_value(), 0);
  }
...
void SCREEN::PageDraw_4() {
  ...
  //только для одиночных страниц:
  //здесь должна быть процедура отрисовки значка игнорирования текущего параметра, если мы установили, что игнорируем его.
